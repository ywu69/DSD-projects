	...
	public void run() {
		try {
			...
			while (!(line = br.readLine().trim()).equals("")) {
				...
				if (line.startsWith("GET /restaurantmashup?city=")) {
					...
					//if (requestline[1].startsWith("/restaurantmashup?city=")) {
						...
						// extract restaurants names and ratings from yelp's
						// search result
						...
						for (int i = 0; i < restaurants.size(); i++) {
							...
							String each_rating = restaurant.get("rating")
									.toString();
							// get twitter reviews
							// String viewResults[] = new
							// String[restaurants.size()];
						...
						// String responsebody = header + body.toString() + tail;
